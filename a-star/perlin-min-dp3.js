...
//		for( w = -_output.width; w < 0; w++ )
		{
			let here = noise.get( 128*w/_output.width +wO, 128*h/_output.height+hO, h2 )*2-1;
			//here = Math.acos(here) / (Math.PI*2);
			var c1,c2,c3;
...
//			if( toHerex < 0 ) angle_view = 1 - angle_view;
	      ...
	      const hx = Math.sin((here)*2*Math.PI);
			const hy = Math.cos((here)*2*Math.PI);
			...
			const angle = (	Math.acos( dot )-Math.PI/2)/(Math.PI/2);

			const dot2 = hx*hx2 + hy*hy2;
//			const dot2 = toHerex*hx2 + toHerey*hy2;
			
			// angle between my forward and here
			const angle2 = (	Math.acos( dot2 ) )/(Math.PI/2)-1;
...
if(0) {
			...
			//if(0)
			if( here < 0 ) {
				tmpC[0] =  -here * 255;//-dot*100;
				tmpC[1] =  0;//-dot*100;
				tmpC[2] =  0;//-dot*100;
			} else {
					tmpC[0] = 0;//120+dot2*120;
					tmpC[1] =  here * 255;//-dot*100;
					tmpC[2] =  0;//-dot*100;
			}

/*
			if( dot < 0 ) {
				tmpC[0] =  120+ -dot * 120;//-dot*100;
				tmpC[1] =  0;//-dot*100;
				tmpC[2] =  0;//-dot*100;
			} else {
					tmpC[0] = 0;//120+dot2*120;
