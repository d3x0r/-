...
export function SaltyRNG(f, opt) {
	...
	var RNG = {
		...
		reset() {
			...
			if( shabuf )
				shabuf.clean();
			if( k12buf ) {
				console.log( "Re-initting K12 after?" );
				k12buf.init();
			}
		},
		getByte() {
		...
		if( k12buf ) {
			...
			if( k12buf.absorbing() || ( RNG.total_bits >= K12_SQUEEZE_LENGTH ) ) {
				...
				if (typeof (RNG.getSalt) === 'function') {
					...
					console.log( "Update to send?", RNG.saltbuf );
			...
			if( k12buf.squeezing() ) {
				...
				console.log( "And need bits is :", RNG.entropy );
			}
		}
		if( shabuf ) {
			if (typeof (RNG.getSalt) === 'function')
	...
	console.log( "Running first init on an RNG" );
	RNG.reset();
...
function KangarooTwelve() {
	...
	var K12 = {
		...
		squeeze(n) {
			...
			console.trace( "Squeeze?", data.realBuf );
			return data.realBuf;
		},
		release(buf) {
			if(0){
...
function getSalt2 (saltbuf) {
    if( salt ) {
	console.log( "Resulting with additional salt for ID:", salt );
        saltbuf.push( salt );
        salt = null;
    }
}
...
SaltyRNG.id = function( s ) {
	if( s !== undefined ) {
		salt = s;
		console.log( "--------------- RESET, then GET BUFFER " );
		RNG2.reset();
