...
function processMessage( msg, stream ) {
    ...
    if( msg.op === "run"){
        ...
        switch( true ){
            ...
            case "placed" === msg.on:
                msg.args = makeEntity( msg.args );
                onEnt.cache.near.place( msg.args );
                break;
...
var fillSandbox = {
    ...
    , leave(...args) { return self.post("leave",null,...args);  }
    , enter(...args) { return self.post("enter",null,...args);  }
    , grab(thing){ doLog( "This grabbing something..." ); return entity.post("grab",thing.Λ);  }
    , drop(...args) { return self.post("drop",null,...args); }
    , store(...args) { return self.post("store",null,...args); }
    //, crypto: crypto
    ...
    , get holds()  { return self.postGetter("holding") }
    , get container() { return self.postGetter("container") }
    , get near()  {
        return self.postGetter("near").then( nearList=>{
            nearList.forEach( (near,i)=>{
                nearList[i] = makeEntity( near );
            } )
            return Promise.resolve( nearList );
        } )
    }
