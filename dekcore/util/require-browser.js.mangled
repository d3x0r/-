...
if( typeof location === "undefined" )
	var location = { href:"./" };
var parser = URL ? URL(location.href,true) : document.createElement('A');
...
function resolve(identifier) {
	...
	var root = m[2] ? requirePath[p[1]?parseInt(p[1]):0] : requirePath[m[1]?parseInt(m[1]):0];
        console.log( "root is: ", root, requirePath );
...
require.oldRequire = self.require;
delete self.require;
